...
export async function getRevenueMetrics(month: number, year: number) {
  ...
  const outstandingBalance = unpaidInvoices.reduce((sum, i) => sum + i.amount, 0);
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const historicalStart = new Date(historicalMonth.getFullYear(), historicalMonth.getMonth(), 1);
    const historicalEnd = new Date(historicalMonth.getFullYear(), historicalMonth.getMonth() + 1, 0, 23, 59, 59, 999);

    const mInvs = await prisma.invoice.findMany({
       where: { issuedAt: { gte: historicalStart, lte: historicalEnd }, status: 'PAID' }
    });
    const rev = mInvs.reduce((sum, inv) => sum + inv.amount, 0);
    history.push({ name: mName, total: rev });
  }

  return { mtdRevenue, outstandingBalance, totalPaid, avgValue, invoices, history };
}

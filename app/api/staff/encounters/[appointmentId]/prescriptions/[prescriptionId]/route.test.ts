...
vi.mock('@/modules/encounters/service', () => ({ deletePrescription: vi.fn() }));

const mockRbac = vi.mocked(requireRole);
const params = withParams({ appointmentId: 'a-1', prescriptionId: 'rx-1' });

describe('DELETE /api/staff/encounters/[appointmentId]/prescriptions/[prescriptionId]', () => {
  ...
  it('returns 404 when not found', async () => {
    ...
    vi.mocked(service.deletePrescription).mockResolvedValue({ error: 'NOT_FOUND' } as any);
    await expectJson(await DELETE(makeRequest({ method: 'DELETE' }), params), 404);
  });

  it('returns 409 when already issued', async () => {
    setRbac(mockRbac, { authorized: true, role: 'DOCTOR' });
    vi.mocked(service.deletePrescription).mockResolvedValue({ error: 'ALREADY_ISSUED' } as any);
    await expectJson(await DELETE(makeRequest({ method: 'DELETE' }), params), 409);
  });

  it('deletes successfully', async () => {
    setRbac(mockRbac, { authorized: true, role: 'DOCTOR' });
    vi.mocked(service.deletePrescription).mockResolvedValue({ ok: true } as any);
    const body = await expectJson(await DELETE(makeRequest({ method: 'DELETE' }), params), 200);
    expect(service.deletePrescription).toHaveBeenCalledWith('a-1', 'rx-1');
